...
class FLL2024SubmergedMissions(FLLBaseLib):
    ...
    async def mission_13_CSL_20(self):
        await self.move_forward(40,velo=500,steer=-2)
        arm_up=False
        if arm_up:
            await self.turn_right(79,speed=100)
        else:
            await self.turn_right(77,speed=100) #73
        ...
        if arm_up:
            mvfwd=13 #11
        await self.move_forward(mvfwd,steer=4)
    ...
    async def mission_shark_working(self):
        if True:
            await self.move_forward(76,700,1)
            await self.turn_left(76)
    ...
    async def mission_5_angular_fish(self):
        ...
        await self.turn_right(33,speed=100,use_gyro=True)
        await self.second_arm_down()
        await self.move_forward(50,500)
    ...
    async def race1(self):
        #await self.mission_1_coral_nursery_40_10()
    ...
    async def race5(self):
        await self.mission_11_UE_20_10()
        pass
    ...
    async def test(self):
        await self.turn_left(90,use_gyro=True)
        return
        await self.first_arm_reset()
