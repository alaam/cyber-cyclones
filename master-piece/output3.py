...
class Cyber_Cylones(FLLBaseLib):
    ...
    # 
    # Alignment - Back wheel next to the LEGO label-L and horizontal straight in line with LEGO.
    ################################################################
    async def mission9(self):
                
        await self.move_forward(22)  # Move Forward to reach the camera location.
        
        await cc_robot.lefthand_movedown_relative(312) # Move down the attachment to pickup camera.

        await self.move_backward(15) # Move backward to make position to place the camera into target area.
        
        await self.turn_left(37, 200) # camera is touching the mat and is at least partly in the camera target area.
        
        await self.move_forward(3, 260) # Move forward to get untangle the camera.
        
        await cc_robot.lefthand_moveup_relative(310,180) # Lift the left hand up.
                     
        await self.move_backward(10) # Bring the Robot back to home
