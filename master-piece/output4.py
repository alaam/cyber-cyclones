...
class Cyber_Cylones(FLLBaseLib):
    ...
    # Mission 10 - Sound Mixer######################################
    # Alignement : ????
    ################################################################
    async def mission10(self):
        # Wait make sure the attachement is all the way up before starting this mission #
        #await cc_robot.lefthand_movedown_relative(360) # Move down the attachment to start
        await self.move_forward(34)# Move the Robot forward 14 centmeter to the mission area.
        await cc_robot.lefthand_moveup_relative(180,200) # Move up the attachment to lift up the sound mixer buttons.
        await self.move_backward(34) # Bring Robot back to home/launch area.
        await cc_robot.lefthand_movedown_relative(180,200)
